...
[[nodiscard]] std::string readZipString(std::span<const Byte> data, std::size_t start, std::size_t length,
...
[[nodiscard]] std::string parseFirstZipFilename(std::span<const Byte> archive_data) {
	...
	const std::string filename = readZipString(
		archive_data,
		FILENAME_INDEX,
		filename_length,
...
void validateArchiveEntryPaths(std::span<const Byte> archive_data) {
	...
	for (uint16_t i = 0; i < total_records; ++i) {
		...
		const std::string entry_name = readZipString(
			archive_data,
			name_start,
			name_length,
		...
		const std::string local_entry_name = readZipString(
			archive_data,
			local_name_start,
			local_name_length,
			"Archive File Error: Local filename length overflow.",
