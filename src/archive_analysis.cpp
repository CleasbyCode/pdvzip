...
[[nodiscard]] std::string parseFirstZipFilename(std::span<const Byte> archive_data) {
	...
	if (containsControlCharacters(filename)) {
		throw std::runtime_error("Archive File Error: First filename contains unsupported control characters.");
	}
	if (isUnsafeEntryPath(filename)) {
...
void validateArchiveEntryPaths(std::span<const Byte> archive_data) {
	...
	for (uint16_t i = 0; i < total_records; ++i) {
		...
		const std::size_t local_header_offset = readLe32(archive_data, cursor + CENTRAL_LOCAL_OFFSET, "Archive File Error");
		...
		const std::size_t name_end = checkedAdd(
			name_start,
			name_length,
		...
		if (containsControlCharacters(entry_name)) {
			throw std::runtime_error(std::format(
				"Archive File Error: Entry {} contains unsupported control characters.", i + 1));
		}
		...
		const std::size_t local_name_length = readLe16(archive_data, local_header_start + 26, "Archive File Error");
		const std::size_t local_extra_length = readLe16(archive_data, local_header_start + 28, "Archive File Error");
		const std::size_t local_name_start = checkedAdd(
			local_header_start,
			LOCAL_RECORD_NAME_INDEX,
			"Archive File Error: Local filename offset overflow.");
		const std::size_t local_name_end = checkedAdd(
			local_name_start,
			local_name_length,
		...
		if (local_name_end > archive_data.size() || local_record_end > archive_data.size()) {
			throw std::runtime_error("Archive File Error: Local file header exceeds archive bounds.");
		}

		const std::string local_entry_name(
			reinterpret_cast<const char*>(archive_data.data() + local_name_start),
			local_name_length);
		if (containsControlCharacters(local_entry_name)) {
			throw std::runtime_error(std::format(
				"Archive File Error: Local entry {} contains unsupported control characters.", i + 1));
		}
		if (isUnsafeEntryPath(local_entry_name)) {
			throw std::runtime_error(std::format(
				"Archive Security Error: Unsafe local archive entry path detected: \"{}\".", local_entry_name));
		}
		...
		const std::size_t record_variable_size = checkedAdd(
			name_length,
			checkedAdd(
				extra_length,
				comment_length,
