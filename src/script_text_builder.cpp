...
#define LINUX_EXTRACT_ITEM R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_$$";clear;mkdir "$DIR"||exit;mv -- "$0" "$DIR"||exit;cd "$DIR"||exit;unzip -qo -- "$SELF"||exit;)"
#define LINUX_EXTRACT_ITEM_HASH R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_$$";clear;mkdir "$DIR"||exit;mv -- "$0" "$DIR"||exit;cd "$DIR"||exit;unzip -qo -- "$SELF"||exit;hash -r;)"
#define LINUX_EXTRACT_ITEM_HASH_NULL R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_$$";NUL="/dev/null";clear;mkdir "$DIR"||exit;mv -- "$0" "$DIR"||exit;cd "$DIR"||exit;unzip -qo -- "$SELF"||exit;hash -r;)"
#define LINUX_EXTRACT_NO_ITEM R"(SELF=$(basename -- "$0");DIR="pdvzip_$$";clear;mkdir "$DIR"||exit;mv -- "$0" "$DIR"||exit;cd "$DIR"||exit;unzip -qo -- "$SELF"||exit;)"
#define WINDOWS_EXTRACT R"(#&cls&setlocal EnableDelayedExpansion&set "DIR=pdvzip_!RANDOM!"&mkdir ".\!DIR!"||exit /b&move "%~dpnx0" ".\!DIR!"||exit /b&cd ".\!DIR!"||exit /b&cls&tar -xf "%~n0%~x0"||exit /b&ren "%~n0%~x0" *.png&)"
#define WINDOWS_PYTHON_EXTRACT R"(#&cls&setlocal EnableDelayedExpansion&set "APP=python3"&set "DIR=pdvzip_!RANDOM!"&mkdir ".\!DIR!"||exit /b&move "%~dpnx0" ".\!DIR!"||exit /b&cd ".\!DIR!"||exit /b&cls&tar -xf "%~n0%~x0"||exit /b&ren "%~n0%~x0" *.png&)"
#define WINDOWS_POWERSHELL_EXTRACT R"(#&cls&setlocal EnableDelayedExpansion&set "PDIR=%SystemDrive%\Program Files\PowerShell\"&set "DIR=pdvzip_!RANDOM!"&mkdir ".\!DIR!"||exit /b&move "%~dpnx0" ".\!DIR!"||exit /b&cd ".\!DIR!"||exit /b&cls&tar -xf "%~n0%~x0"||exit /b&ren "%~n0%~x0" *.png&)"
...
void validateScriptInputs(const std::string& first_filename, const UserArguments& user_args) {
...
[[nodiscard]] std::string_view combinedArgumentsRaw(const UserArguments& user_args) {
	return user_args.linux_args.empty()
		? std::string_view(user_args.windows_args)
		: std::string_view(user_args.linux_args);
}

[[nodiscard]] std::array<PlaceholderReplacement, 6> makePlaceholderReplacements(
	const std::string& first_filename,
	const UserArguments& user_args
) {
	const std::string_view args_combined_raw = combinedArgumentsRaw(user_args);

	return std::array<PlaceholderReplacement, 6>{{
		{ TOKEN_LINUX_FILENAME_ARG, quotePosixArgument(makePosixCommandPath(first_filename)) },
		{ TOKEN_WINDOWS_FILENAME_ARG, quoteWindowsArgumentForCmd(makeWindowsCommandPath(first_filename)) },
		{ TOKEN_LINUX_ARGS, renderPosixArguments(user_args.linux_args, "Linux arguments") },
		{ TOKEN_WINDOWS_ARGS, renderWindowsArguments(user_args.windows_args, "Windows arguments") },
		{ TOKEN_LINUX_ARGS_COMBINED, renderPosixArguments(args_combined_raw, "Combined Linux arguments") },
		{ TOKEN_WINDOWS_ARGS_COMBINED, renderWindowsArguments(args_combined_raw, "Combined Windows arguments") },
	}};
}
...
std::string buildScriptText(
	...
	validateScriptInputs(first_filename, user_args);
	const ScriptTemplate script_template = getScriptTemplate(file_type);
	const std::array<PlaceholderReplacement, 6> replacements = makePlaceholderReplacements(first_filename, user_args);
