...
// Extract from the original location before moving the polyglot. On a filename
// collision, keep both the source and the extracted member. Check explicitly
// because mv -n's skip status varies across versions; -nT also protects against
// a destination appearing after the check or being treated as a directory.
// Prefix a leading dash in the archive path: UnZip's option parser can still
// interpret such filenames as options even when they follow --.
#define LINUX_EXTRACT R"(clear;mkdir -- "$PATH_DIR"||exit;unzip -qo -d "$PATH_DIR" -- "${0/#-/./-}"||exit;[ -e "$PATH_DIR/$SELF" ]||[ -L "$PATH_DIR/$SELF" ]||mv -nT -- "$0" "$PATH_DIR/$SELF"||exit;cd "$PATH_DIR"||exit;)"
#define LINUX_EXTRACT_ITEM R"(ITEM={{LINUX_FILENAME_ARG}};)" LINUX_EXTRACT_SETUP LINUX_EXTRACT
#define LINUX_EXTRACT_ITEM_HASH LINUX_EXTRACT_ITEM R"(hash -r;)"
#define LINUX_EXTRACT_ITEM_HASH_NULL R"(ITEM={{LINUX_FILENAME_ARG}};)" LINUX_EXTRACT_SETUP R"(NUL="/dev/null";)" LINUX_EXTRACT R"(hash -r;)"
#define LINUX_EXTRACT_NO_ITEM LINUX_EXTRACT_SETUP LINUX_EXTRACT
...
std::vector<std::string> splitPosixArguments(std::string_view input, std::string_view field_name) {
	...
	for (char ch : input) {
		...
		if (escaped) {
			// Within double quotes, only $, `, ", and backslash are escaped.
			// Keep the backslash before other characters in these single-line
			// arguments; script rendering rejects control characters separately.
