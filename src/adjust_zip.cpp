...
void adjustZipOffsets(std::vector<uchar>& Image_Vec, const uint32_t LAST_IDAT_CHUNK_NAME_INDEX) {
	...
	const uint32_t
		START_CENTRAL_DIR_INDEX = static_cast<uint32_t>(std::search(Image_Vec.begin() + LAST_IDAT_CHUNK_NAME_INDEX, Image_Vec.end(), 		
		START_CENTRAL_DIR_SIG.begin(), START_CENTRAL_DIR_SIG.end()) - Image_Vec.begin()),
		END_CENTRAL_DIR_INDEX = static_cast<uint32_t>(std::search(Image_Vec.begin() + START_CENTRAL_DIR_INDEX, Image_Vec.end(), END_CENTRAL_DIR_SIG.begin(), END_CENTRAL_DIR_SIG.end()) - Image_Vec.begin());
	...
	while (zip_records--) {
		new_offset = static_cast<uint32_t>(std::search(Image_Vec.begin() + new_offset + 1, Image_Vec.end(), ZIP_SIG.begin(), ZIP_SIG.end()) - Image_Vec.begin()),
		central_local_index = 45 + static_cast<uint32_t>(std::search(Image_Vec.begin() + central_local_index, Image_Vec.end(), START_CENTRAL_DIR_SIG.begin(), START_CENTRAL_DIR_SIG.end()) - Image_Vec.begin());
