...
void fixZipOffsets(vBytes& image_vec, std::size_t original_image_size, std::size_t script_data_size) {
	...
	constexpr std::size_t LEN2 = 2, LEN4 = 4;
	...
	auto eocd_it = std::search(
		image_vec.rbegin(), image_vec.rend(),
		END_CENTRAL_DIR_SIG.rbegin(), END_CENTRAL_DIR_SIG.rend());
	...
	const uint16_t total_records = static_cast<uint16_t>(
		getValue(image_vec, total_records_index, LEN2, LE));
	...
	const uint16_t original_comment_length = static_cast<uint16_t>(
		getValue(image_vec, comment_length_index, LEN2, LE));
	...
	updateValue(image_vec, comment_length_index, new_comment_length, LEN2, LE);
	...
	for (uint16_t i = 0; i < total_records; ++i) {
		search_it = std::search(
			search_it, image_vec.rend(),
			START_CENTRAL_DIR_SIG.rbegin(), START_CENTRAL_DIR_SIG.rend());
		...
		start_central_index = complete_size
			- static_cast<std::size_t>(std::distance(image_vec.rbegin(), search_it++))
			- ZIP_SIG_LENGTH;
	}

	// Update the End Central Directory to point to the Start Central Directory.
	updateValue(image_vec, end_central_start, static_cast<uint32_t>(start_central_index), LEN4, LE);
	...
	for (uint16_t i = 0; i < total_records; ++i) {
		updateValue(image_vec, central_local_index, static_cast<uint32_t>(local_index), LEN4, LE);
		...
		if (i + 1 < total_records) {
			...
			if (!next_local) {
				throw std::runtime_error(std::format(
					"ZIP Error: Local file header {} of {} not found.", i + 2, total_records));
			}
			...
			if (!next_central) {
				throw std::runtime_error(std::format(
					"ZIP Error: Central directory entry {} of {} not found.", i + 2, total_records));
			}
...
void embedChunks(vBytes& image_vec, vBytes script_vec, vBytes archive_vec, std::size_t original_image_size) {
	...
	updateValue(image_vec, crc_index, last_idat_crc, 4);
}
