...
[[nodiscard]] ZipEocdInfo findZipEocd(std::span<const Byte> image_vec, std::size_t zip_base_offset, std::size_t zip_end_offset) {
	...
	const std::size_t distance_floor = (zip_end_offset > MAX_EOCD_SEARCH_DISTANCE)
		? zip_end_offset - MAX_EOCD_SEARCH_DISTANCE
		: std::size_t{0};
	const std::size_t search_floor = std::max(zip_base_offset, distance_floor);
	const std::size_t search_start = zip_end_offset - END_CENTRAL_DIR_SIG.size();
	...
	for (std::size_t pos = search_start; ; --pos) {
		if (std::equal(END_CENTRAL_DIR_SIG.begin(), END_CENTRAL_DIR_SIG.end(), image_vec.begin() + pos)
			&& pos + EOCD_MIN_SIZE <= zip_end_offset) {
			...
			if (pos + EOCD_MIN_SIZE + info.comment_length == zip_end_offset) {
...
void embedChunks(vBytes& image_vec, vBytes script_vec, vBytes archive_vec, std::size_t original_image_size) {
	...
	const std::size_t output_size = checkedAdd(
		checkedAdd(image_vec.size(), script_vec.size(), "Embed Error: Output image size overflow."),
		archive_vec.size(),
