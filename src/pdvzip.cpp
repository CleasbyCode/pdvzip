void startPdv(const std::string& IMAGE_FILENAME, const std::string& ZIP_FILENAME, bool isZipFile) {
	...
	constexpr uint_fast8_t
		...
		LINUX_PROBLEM_CHARACTERS[7] { 0x22, 0x27, 0x28, 0x29, 0x3B, 0x3E, 0x60 },
		MAX_INDEX = 0x20;
	...
	const uint_fast8_t INZIP_FILENAME_LENGTH = Idat_Zip_Vec[34];

	if (!std::equal(std::begin(ZIP_SIG), std::end(ZIP_SIG), std::begin(Idat_Zip_Vec) + 8) || MIN_INZIP_FILENAME_LENGTH > INZIP_FILENAME_LENGTH) {
		std::cerr << "\nZIP File Error: " 
			<< (MIN_INZIP_FILENAME_LENGTH > INZIP_FILENAME_LENGTH 
	...
	if (std::find(std::begin(LINUX_PROBLEM_CHARACTERS), std::end(LINUX_PROBLEM_CHARACTERS), iccp_chunk_length_first_byte_value) != std::end(LINUX_PROBLEM_CHARACTERS)) {
			...
			iccp_chunk_script_size = static_cast<uint_fast32_t>(Iccp_Script_Vec.size() - 12);

			valueUpdater(Iccp_Script_Vec, iccp_chunk_length_index, iccp_chunk_script_size, value_bit_length, true);
	}
	
	uint_fast32_t combined_file_size = static_cast<uint_fast32_t>(iccp_chunk_script_size + IMAGE_VEC_SIZE + IDAT_CHUNK_ZIP_FILE_SIZE);
