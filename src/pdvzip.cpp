...
// The following code (slightly modified) to compute CRC32 for PNG chunks is taken from: https://www.w3.org/TR/2003/REC-PNG-20031110/#D-CRCAppendix 
static size_t crcUpdate(const size_t& crc, uchar* buf, const size_t& len) {
...
static size_t crc(uchar* buf, const size_t& len)
{
...
void startPdv(const std::string& IMAGE_NAME, const std::string& ZIP_NAME, bool isZipFile) {
	...
	bool file_size_check = image_size > MIN_IMAGE_SIZE && zip_size > MIN_ZIP_SIZE && MAX_FILE_SIZE >= combined_file_size;

	if (!file_size_check) {
		std::cerr << "\nFile Size Error: " << (MIN_IMAGE_SIZE > image_size ? "Invalid PNG image. File too small"
			: (MIN_ZIP_SIZE > zip_size ? "Invalid ZIP file. File too small"
	...
	while (ihdr_chunk_index++ != 32) { 
		for (int i = 0; i < 7; i++) {
			if (Image_Vec[ihdr_chunk_index] == LINUX_PROBLEM_CHARACTERS[i]) { 
				std::cerr <<
	...
	const int
		COVER_IMAGE_WIDTH = Image_Vec[18] << 8 | Image_Vec[19],
		COVER_IMAGE_HEIGHT = Image_Vec[22] << 8 | Image_Vec[23],
		COVER_PNG_COLOR_TYPE = Image_Vec[25] == 6 ? 2 : Image_Vec[25];
	...
	const bool
		VALID_COLOR_TYPE = (COVER_PNG_COLOR_TYPE == PNG_INDEXED_COLOR) ? true		
		: ((COVER_PNG_COLOR_TYPE == PNG_TRUECOLOR) ? true : false),

		VALID_IMAGE_DIMS = (COVER_PNG_COLOR_TYPE == PNG_TRUECOLOR			
		...
		: ((COVER_PNG_COLOR_TYPE == PNG_INDEXED_COLOR					
	...
	const size_t
		FIRST_IDAT_CHUNK_LENGTH = ((static_cast<size_t>(Image_Vec[idat_chunk_index]) << 24)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 1]) << 16)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 2]) << 8)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 3]))),

		FIRST_IDAT_CHUNK_CRC_INDEX = idat_chunk_index + FIRST_IDAT_CHUNK_LENGTH + 8,

		FIRST_IDAT_CHUNK_CRC = ((static_cast<size_t>(Image_Vec[FIRST_IDAT_CHUNK_CRC_INDEX]) << 24)
			| (static_cast<size_t>(Image_Vec[FIRST_IDAT_CHUNK_CRC_INDEX + 1]) << 16)
			| (static_cast<size_t>(Image_Vec[FIRST_IDAT_CHUNK_CRC_INDEX + 2]) << 8)
			| (static_cast<size_t>(Image_Vec[FIRST_IDAT_CHUNK_CRC_INDEX + 3]))),

		CALC_FIRST_IDAT_CHUNK_CRC = crc(&Image_Vec[idat_chunk_index + 4], FIRST_IDAT_CHUNK_LENGTH + 4);
	...
	while (image_size != idat_chunk_index + 4) {
		const size_t IDAT_CHUNK_LENGTH = ((static_cast<size_t>(Image_Vec[idat_chunk_index]) << 24)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 1]) << 16)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 2]) << 8)
			| (static_cast<size_t>(Image_Vec[idat_chunk_index + 3])));
	...
	std::vector<std::vector<uchar>> Extraction_Scripts_Vec{
		...
		for (int i = 0; i < 7; i++) {
			if (Iccp_Script_Vec[3] == LINUX_PROBLEM_CHARACTERS[i]) {
		...
		const size_t LAST_IDAT_CHUNK_CRC = crc(&Image_Vec[LAST_IDAT_CHUNK_NAME_INDEX], zip_size - 8);
