void startPdv(const std::string& IMAGE_FILENAME, const std::string& ZIP_FILENAME, bool isZipFile) {
	...
	checkFileSizes(IMAGE_FILENAME, ZIP_FILENAME);
	...
	checkImageSignatures(Image_Vec);
	
	const uint32_t IMAGE_VEC_SIZE = getVecSize(Image_Vec);
	...
	checkZipSignature(Idat_Zip_Vec);

	const uint32_t IDAT_CHUNK_ZIP_FILE_SIZE = getVecSize(Idat_Zip_Vec);
	...
	uint8_t 
		...
		iccp_chunk_length_first_byte_value_index = 3;
	
	uint32_t iccp_chunk_script_size = getVecSize(Iccp_Script_Vec) - 12;

	valueUpdater(Iccp_Script_Vec, iccp_chunk_length_index, iccp_chunk_script_size, value_bit_length, true);

	const uint8_t iccp_chunk_length_first_byte_value = Iccp_Script_Vec[iccp_chunk_length_first_byte_value_index];

	// If a problem character (breaks Linux extraction script) is found within the first byte of the updated iccp chunk length field, 
	...
	if ((iccp_chunk_length_first_byte_value > 33 && iccp_chunk_length_first_byte_value < 42) || 
	    (iccp_chunk_length_first_byte_value > 58 && iccp_chunk_length_first_byte_value < 63) ||
	    (iccp_chunk_length_first_byte_value == 96)) {
	...
	const uint32_t LAST_IDAT_CHUNK_NAME_INDEX = IMAGE_VEC_SIZE + iccp_chunk_script_size + 4;
	...
	writeFile(Image_Vec, COMPLETE_POLYGLOT_IMAGE_SIZE, isZipFile);
}
