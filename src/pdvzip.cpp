// 	PNG Data Vehicle, ZIP Edition (PDVZIP v2.1). Created by Nicholas Cleasby (@CleasbyCode) 6/08/2022
...
void
	...
	Erase_Image_Chunks(PDV_STRUCT&, std::ifstream&), // Only keep critical chunks from PNG cover image.
...
void Erase_Image_Chunks(PDV_STRUCT& pdv, std::ifstream& zip_ifs) {
	...
	// For PNG cover image, just keep the critical PNG chunks. 
...
void Check_Zip_File(PDV_STRUCT& pdv, std::ifstream& zip_ifs) {
	...
	// Vector "Idat_Zip_Vec" will store the user's ZIP file. The contents of the vector will later be inserted into the "Image_Vec" vector as the last IDAT chunk. 
	...
	Value_Updater(pdv.Idat_Zip_Vec, idat_chunk_length_index, pdv.zip_size - 12, value_length_bits, pdv.big_endian);
...
void Complete_Extraction_Script(PDV_STRUCT& pdv) {
	...
	std::vector<std::vector<Byte>> Extraction_Scripts_Array_Vec {
		...
		// Script Vector 8 Default. For unmatched file extensions. Rely on Linux/Windows default application to deal with these files, if possible.
		{ 0x49, 0x54, 0x45, 0x4D, 0x3D, 0x22, 0x22, 0x3B, 0x44, 0x49, 0x52, 0x3D, 0x22, 0x70, 0x64, 0x76, 0x7A, 0x69, 0x70, 0x5F, 0x65, 0x78, 0x74, 0x72, 0x61, 0x63,
		  0x74, 0x65, 0x64, 0x22, 0x3B, 0x63, 0x6C, 0x65, 0x61, 0x72, 0x3B, 0x6D, 0x6B, 0x64, 0x69, 0x72, 0x20, 0x2D, 0x70, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22,
		  0x3B, 0x6D, 0x76, 0x20, 0x22, 0x24, 0x30, 0x22, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22, 0x3B, 0x63, 0x64, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22, 0x3B,
		  0x75, 0x6E, 0x7A, 0x69, 0x70, 0x20, 0x2D, 0x71, 0x6F, 0x20, 0x22, 0x24, 0x30, 0x22, 0x3B, 0x78, 0x64, 0x67, 0x2D, 0x6F, 0x70, 0x65, 0x6E, 0x20, 0x22, 0x24,
	...
	// Vector "Case_Values_Vec", will contain between 3 and 5 values depending on the matched file extension.
	// The first value in the vector is always one of the Script Vectors within the vector array "Extraction_Scripts_Array_Vec".
	// The remaining values are insert index locations within the Script Vector for elements to be inserted, 
	// such as filenames & command arguments, which completes the extraction script.
	...
	else if (extension_index == PYTHON || 
				extension_index == POWERSHELL || 
					extension_index == BASH_SHELL) {
						Iccp_Script_Vec.insert(Iccp_Script_Vec.begin() + Case_Values_Vec[1], args_windows.begin(), args_windows.end());
						Iccp_Script_Vec.insert(Iccp_Script_Vec.begin() + Case_Values_Vec[2], first_zip_name.begin(), first_zip_name.end());
	...
	size_t iccp_chunk_script_size = Iccp_Script_Vec.size() - 12;

	size_t iccp_chunk_length_index = 2;

	int value_length_bits = 16;
	...
	Value_Updater(Iccp_Script_Vec, iccp_chunk_length_index, iccp_chunk_script_size, value_length_bits, pdv.big_endian);
	...
	for (int i = 0; i < 7; i++) {
		if (Iccp_Script_Vec[3] == pdv.LINUX_PROBLEM_CHARACTERS[i]) {
			...
			Value_Updater(Iccp_Script_Vec, iccp_chunk_length_index, iccp_chunk_script_size, value_length_bits, pdv.big_endian); // Update size again.
	...
	Value_Updater(Iccp_Script_Vec, iccp_chunk_crc_index, ICCP_CHUNK_CRC, value_length_bits, pdv.big_endian);
	...
	Value_Updater(pdv.Image_Vec, last_idat_chunk_crc_index, LAST_IDAT_CHUNK_CRC, value_length_bits, pdv.big_endian);
...
void Fix_Zip_Offset(PDV_STRUCT& pdv, const size_t& LAST_IDAT_CHUNK_NAME_INDEX) {
	...
	// To run an embedded JAR file, you will need to rename the '.png' extension to '.jar'.  
...
void Write_Polyglot_File(PDV_STRUCT& pdv) {
	...
	std::cout << "\nCreated PNG-ZIP polyglot image file: " + PDV_FILENAME + '\x20' + std::to_string(pdv.image_size) + " Bytes.\n\nComplete!\n\nYou can now post this image on the relevant supported platforms.\n\n";
}
