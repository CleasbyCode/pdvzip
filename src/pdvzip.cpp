...
// Read-in user PNG image & ZIP file. Store them in separate vectors.
void storeFiles(std::ifstream&, std::ifstream&, const std::string&, const std::string&);

// Make sure user PNG image & ZIP file fulfil valid program requirements. Display relevant error message & exit program if checks fail.
void checkFileRequirements(std::vector<BYTE>&, std::vector<BYTE>&, const char(&)[]);

// Search & remove all unnecessary PNG chunks found before the first IDAT chunk.
void eraseChunks(std::vector<BYTE>&);

// Search for & change problematic characters within the PNG palette chunk to prevent pdvzip's Linux extraction script from breaking.
void modifyPaletteChunk(std::vector<BYTE>&, const char(&)[]);

// Update pdvzip's barebones extraction script determined by embedded content. 
void completeScript(std::vector<BYTE>&, std::vector<BYTE>&, const char(&)[]);

// Insert contents of vectors storing ZIP file & the completed extraction script into the vector containing PNG image. Write vector's content to file.
...
// Code to compute CRC32 (for IHDR, PLTE & IDAT chunks within this program) was taken from: https://www.w3.org/TR/2003/REC-PNG-20031110/#D-CRCAppendix 
...
int main(int argc, char** argv) {
	...
	if (argc == 2 && std::string(argv[1]) == "--info") {
		...
		// Display relevant error message & exit program if a file fails to open.
		if (!IMAGE || !ZIP) {
			const std::string
...
void storeFiles(std::ifstream& IMAGE, std::ifstream& ZIP, const std::string& IMG_FILE, const std::string& ZIP_FILE) {
	...
	// Vector "ImageVec" stores the user's PNG image. Combined later with the vectors "ScriptVec" & "ZipVec", before writing vector "ImageVec" out to file.

	std::vector<BYTE>
		ZipVec{ 0,0,0,0,73,68,65,84,0,0,0,0 }, // IDAT chunk name with 4-byte chunk length & crc fields.
		ImageVec((std::istreambuf_iterator<char>(IMAGE)), std::istreambuf_iterator<char>());  // Insert user image file into ImageVec.

	// Get size of user's ZIP file.
	ZIP.seekg(0, ZIP.end);
	...
	// Insert user's ZIP file into vector "ZipVec".
	...
	// Occurrence of these characters in the IHDR chunk (data/crc), PLTE chunk (data/crc) or hIST chunk (length), breaks pdvzip's Linux extraction script.
	static const char BAD_CHAR[7]{ '(', ')', '\'', '`', '"', '>', ';' };

	// Make sure PNG & ZIP file specs meet program requirements. 
	checkFileRequirements(ImageVec, ZipVec, BAD_CHAR);

	const size_t
		IMG_SIZE = ImageVec.size(),
		MAX_PNG_SIZE_BYTES = 209715200,	// 200MB PNG file size limit for pdvzip. Compatible hosting sites have various size limts (e.g. 5MB for Twitter).
		MAX_SCRIPT_SIZE_BYTES = 750,	// pdvzip's extraction script size limit.
		COMBINED_SIZE = IMG_SIZE + ZIP_SIZE + MAX_SCRIPT_SIZE_BYTES;

	// Check size of files. Display relevant error message & exit program if files exceed size limits.
...
void checkFileRequirements(std::vector<BYTE>& ImageVec, std::vector<BYTE>& ZipVec, const char(&BAD_CHAR)[]) {
	...
	if (IMG_VEC_HDR != PNG_SIG
		...
		|| !VALID_COLOUR_TYPE
		|| !VALID_IMAGE_DIMS
		|| MIN_NAME_LENGTH > INZIP_NAME_LENGTH) { // Requirements check failure. Display relevant error message & exit program.
	...
	// Find & remove unnecessary PNG chunks.
        eraseChunks(ImageVec);

	// Check if image is PNG-8 format (Indexed-colour / Colour type: 3).
	if (COLOUR_TYPE == PNG_INDEXCOLOUR) {
		// Alter problematic characters within the PLTE chunk to prevent pdvzip's Linux extraction script from breaking.
		modifyPaletteChunk(ImageVec, BAD_CHAR);
	}

	// Check a range of bytes in the IHDR chunk to make sure ther are no problematic characters that breaks pdvzip's Linux extraction script.
	// A problematic character can appear within the width & height fields or the crc field of the IHDR chunk.
	// Manually modifying the dimensions (1% increase or decrease) of the image will usually resolve the issue. Repeat if necessary.

	TBYTE ihdrChunkPos = 18; // Vector index start position of IHDR chunk within vector "ImageVec".

	// From ihdrChunkPos location, increment through 15 bytes of ImageVec and compare each byte to the 7 characters within BAD_CHAR.
	for (TBYTE i{}; i < 16; i++) {
		for (TBYTE j{}; j < 7; j++) {
			if (ImageVec[ihdrChunkPos] == BAD_CHAR[j]) { // Problematic character found, display error message & exit program.
				std::cerr <<
					"\nInvalid Character Error:  \n\nThe IHDR chunk of this image contains a character that will break pdvzip's Linux extraction script."
					"\nTry modifying image dimensions (1% increase or decrease) to resolve the issue. Repeat if necessary.\n"
					"\nOnce you have modified the dimensions, retry pdvzip with this image.\n\n";
				std::exit(EXIT_FAILURE);
			}
		}
		ihdrChunkPos++;
...
void eraseChunks(std::vector<BYTE>& ImageVec) {
	...
	for (TBYTE chunkIndex = 14; chunkIndex > 0; chunkIndex--) {
		size_t
			...
			// From last to first, search & get index location of each chunk to remove.
...
void modifyPaletteChunk(std::vector<BYTE>& ImageVec, const char(&BAD_CHAR)[]) {
	...
	// Some individual characters, a sequence or combination of certain characters that may appear within the PLTE chunk, breaks pdvzip's Linux extraction script.
	// This function contains a number of fixes for this issue.
	// The PLTE chunk must be located before the hIST chunk (pdvzip's extraction script) for pdvzip images to work correctly on the Imgur image hosting site.
	...
	for (size_t i = PLTE_CHUNK_INDEX; i < (PLTE_CHUNK_INDEX + (PLTE_CHUNK_SIZE + 4)); i++) {
		...
		// Individual BAD_CHAR characters within PLTE chunk that will break pdvzip's Linux extraction script.
		...
		// Character combinations that will break pdvzip's Linux extraction script. 
		if ((ImageVec[i] == '&' && ImageVec[i + 1] == '!')		// e.g. "&!"
			...
			|| (ImageVec[i] == '<' && ImageVec[i + 1] == ')'))
		{
			// Replace character from the above matched combinations with the relevant GOOD_CHAR to prevent script failure.
			if (ImageVec[i] == '\x0a') {
		...
		// Character "<" followed by a number (or a sequence of numbers up to 11 digits) & ending with the same character "<", breaks pdvzip's Linux extraction script. 
		// Examples, "<1<, <4356<, <293459< <35242369849<, etc".
		TBYTE j = 1, k = 2;
		if (ImageVec[i] == '<') { // Found this "<" character, now check if it is followed by a number & ends with another "<" character.
			while (j < 12) {
	...
	do {
		...
		// Make sure the new crc value does not contain any of the BAD_CHAR characters, which will break pdvzip's Linux extraction script.
		...
		// Repeat process until no problematic characters found.
		for (TBYTE i = 0; i < 5; i++) {
			for (TBYTE j = 0; j < 7; j++) {
				if (i > 3) break;
...
void completeScript(std::vector<BYTE>& ZipVec, std::vector<BYTE>& ImageVec, const char(&BAD_CHAR)[]) {
	...
	First 4-bytes of the vector is the chunk length field, followed by chunk name (hIST) then pdvzip's barebones extraction script.
	...
	command based on a matched file extension. If no match found, defaulting to the operating system making the choice.
	...
	const TBYTE
		INZIP_NAME_LENGTH_INDEX = 34,	// "ZipVec" vector's index location for length value of the zipped media filename.
		...
		BASH_XDG_OPEN = 25,	// "xdg-open" Linux command, runs shell scripts (.sh), opens folders & used for unmatched file extensions.
	...
	std::string
		...
		// Get the file extension from the zipped media filename.
	...
	// For example, in the 1st sequence, sequence[0] = index 236 of vector "ScriptVec" ("insertIndex") corresponds with
	// sequence[5] = vector "AppVec" index 33 ("appIndex"), which is the zipped media filename string element. 
	// Vector "AppVec" string element 33 (zipped media filename) will be inserted into the script (vector "ScriptVec") at index 236.
	...
	TBYTE sequence[52]{
				...
				appIndex = 0,		// Uses vector "AppVec" index values from the sequence array.
				insertIndex = 0,	// Uses vector "ScriptVec" index values from the sequence array.
	...
	/*  [sequence](insertIndex)[sequence](appIndex)
		...
		VIDEO_AUDIO:
		[0]>(236)[5]>(33) Windows: 236 insert index for zipped media filename, vector "AppVec" element 33.
		[1]>(234)[6]>(28) Windows: 234 insert index for "start /b", vector "AppVec" element 28.
		[2]>(116)[7]>(27) Linux: 116 insert index for "Dev Null", vector "AppVec" element 27.
		[3]>(115)[8]>(33) Linux: 115 insert index for zipped media filename, vector "AppVec" element 33.
		[4]>(114)[9]>(20) Linux: 114 insert index for "vlc" vector "AppVec" element 20.
		Sequence limit is 5 (value taken from first appIndex value for each sequence).

		Matching a file extension from "inzipNameExt" with vector "AppVec", we can select which application string and commands to use
		in our extraction script. When executed, will open/play/run (depending on file type) the extracted zipped media file.
	...
	for (; appIndex != 26; appIndex++) {
		if (AppVec[appIndex] == inzipNameExt) {
			// After a file extension match, any appIndex value between 0 & 14 defaults to "AppVec" element 20 (vlc / VIDEO_AUDIO).
			// If over 14, we add 6 to the value. 15 + 6 = vector "AppVec" element 21 (evince/PDF), 16 + 6 = vector "AppVec" element 22 (python3/.py), etc.
	...
	// Provide the user with the option to add command-line arguments for file types: .py, .ps1, .sh & .exe
	if (appIndex > 21 && appIndex < 26) {
		...
		AppVec.push_back(argsLinux),	// Vector "AppVec" element (34).
		AppVec.push_back(argsWindows);	// Vector "AppVec" element (35).
	}
	...
	case POWERSHELL:
		if (appIndex == POWERSHELL) {
			...
			AppVec.push_back(inzipName);		// add the modified filename to the vector "AppVec" element (36).
	...
	case BASH_XDG_OPEN:
		...
		break;
	default:			// Unmatched file extensions. Rely on operating system to use set default program for dealing with this file type.
	...
	// Set the sequenceLimit using the first appIndex variable value from each switch case sequence.
	// Reduce sequenceLimit value by 1 if insertIndex variable is 33 (case BASH_XDG_OPEN).
	sequenceLimit = insertIndex == 33 ? appIndex - 1 : appIndex;

	// With just a single command within the "While-Loop", we can insert all the required strings into the extraction script (vector "ScriptVec"), 
	...
	do {
		...
		// Display relevant error message & exit program if extraction script exceeds size limit.
		...
		// any of the problematic characters that will break pdvzip's Linux extraction script.
		for (TBYTE i = 0; i < 7; i++) {
			if (ScriptVec[3] == BAD_CHAR[i]) {
				// BAD_CHAR found. Insert a byte at the end of vecor "ScriptVec" to increase chunk length. Update chunk length field. 
				// Check again for a BAD_CHAR match. Repeat the byte insertion & chunk length update, until no BAD_CHAR match.
	...
	// Insert the vectors "ScripVec" (hIST chunk/completed extraction script) & "ZipVec" (IDAT chunk/ZIP file) into vector "ImageVec" (PNG image).
...
void combineVectors(std::vector<BYTE>& ImageVec, std::vector<BYTE>& ZipVec, std::vector<BYTE>& ScriptVec) {
	...
	// Insert vector "ScriptVec" (hIST chunk/extraction script) into vector "ImageVec".	
	ImageVec.insert((ImageVec.begin() + FIRST_IDAT_INDEX), ScriptVec.begin(), ScriptVec.end());

	// Insert vector "ZipVec" (IDAT chunk/ZIP file) into vector "ImageVec".
	...
	// Generate crc value for the last IDAT chunk.
	// The +4 value points LAST IDAT index value to the chunk name location, which is where crc calculation needs to include/begin.
...
// The following code (slightly modified) to compute CRC32 (for IHDR, PLTE & IDAT chunks) was taken from: https://www.w3.org/TR/2003/REC-PNG-20031110/#D-CRCAppendix 
