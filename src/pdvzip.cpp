// 	PNG Data Vehicle, ZIP/JAR Edition (PDVZIP v4.2)
...
Paint.net application is recommended for easily creating compatible PNG image files.
	...
	struct ProgramArgs {
		...
		static ProgramArgs parse(int argc, char** argv) {
			...
			if (argc == 2 && std::string_view(argv[1]) == "--info") {
				return ProgramArgs{.image_file_path = std::nullopt, .archive_file_path = std::nullopt, .info_mode = true};
			}
			if (argc != 3) die();
			return ProgramArgs{.image_file_path   = argv[1], .archive_file_path = argv[2], .info_mode = false};
		}
	};

	[[nodiscard]] std::expected<std::size_t, std::string> searchSig(std::span<const Byte> data, std::span<const Byte> sig, std::size_t start_index = 0, std::size_t increment = 0) {
	...
	void resizeImage(vBytes& image_file_vec) {
		...
		for (unsigned y = 0; y < NEW_HEIGHT; ++y) {
			for (unsigned x = 0; x < NEW_WIDTH; ++x) {
				const float
					ORIG_X = std::clamp((x + SAMPLING_OFFSET) * X_RATIO - SAMPLING_OFFSET, 0.0f, static_cast<float>(width  - 1)),
					ORIG_Y = std::clamp((y + SAMPLING_OFFSET) * Y_RATIO - SAMPLING_OFFSET, 0.0f, static_cast<float>(height - 1));
	...
	void imageCheck(vBytes& image_file_vec) {
		constexpr uint16_t
			MIN_RGB_COLORS 		   = 257,
		...
		if ((color_type == TRUECOLOR_RGB || color_type == TRUECOLOR_RGBA) && (MIN_RGB_COLORS > stats.numcolors)) {
			...
			// Make a copy of the cover image, only keep required chunks.
			constexpr std::size_t
				PNG_FIRST_BYTES = 33,
				PNG_IEND_BYTES  = 12;
		...
		auto hasProblemCharacter = [&]() {
			return std::ranges::any_of(image_file_vec | std::views::drop(IHDR_START_INDEX) | std::views::take(IHDR_STOP_INDEX - IHDR_START_INDEX),[](Byte b) { return std::ranges::contains(LINUX_PROBLEM_CHARACTERS, b);});
		};
		...
		color_type = image_file_vec[COLOR_TYPE_INDEX] == TRUECOLOR_RGBA ? TRUECOLOR_RGB : image_file_vec[COLOR_TYPE_INDEX];   // Color type may of changed, update.
...
int main(int argc, char** argv) {
	try {
			...
			const std::string ARC_RECORD_FIRST_FILENAME =
				archive_file_vec
				| std::views::drop(ARC_RECORD_FIRST_FILENAME_INDEX)
				| std::views::take(ARC_RECORD_FIRST_FILENAME_LENGTH)
			...
			// Check filenames that don't have extensions, such as folders and Linux executables.
			...
			auto hasBalancedQuotes = [](std::string_view args_string) {
				...
				for (std::size_t i = 0; i < args_string.size(); ++i) {
					...
					const bool IS_ESCAPED = (i > 0 && args_string[i - 1] == '\\');
			...
			// insert a short string to the end of the iCCP data chunk to increase its length, avoiding the problem characters when chunk length is updated.
			...
			// Find the "last" (first) start_central directory index location by searching the vector backwards.
			...
			// in avoiding the increased probability of a "false-positive" signature match if we were to search the vector from the front/beginning and read through image & file data first.
			...
			std::size_t central_dir_local_index = start_central_dir_index + CENTRAL_LOCAL_INDEX_DIFF; // First central dir index location.

			value_byte_length = 4;
			updateValue(image_file_vec, end_central_start_index, start_central_dir_index, value_byte_length, isBigEndian);  // Update end_central index location with start_central directory offset.
	
			constexpr std::size_t INCREMENT_SEARCH_INDEX = 1;

			while (total_zip_records--) {
				updateValue(image_file_vec, central_dir_local_index, zip_record_local_index, value_byte_length, isBigEndian); 					// Write new zip_record offset to the current central_dir_local index.
				auto zip_result_opt  	= searchSig(image_file_vec, ZIP_LOCAL_SIG, zip_record_local_index, INCREMENT_SEARCH_INDEX); 	  		// Get the next zip_record index/offset.
				zip_record_local_index  = *zip_result_opt;
				auto central_result_opt = searchSig(image_file_vec, START_CENTRAL_DIR_SIG, central_dir_local_index, INCREMENT_SEARCH_INDEX);	// Get the Next central_dir index
