// 	PNG Data Vehicle, ZIP Edition (PDVZIP v2.0). Created by Nicholas Cleasby (@CleasbyCode) 6/08/2022
...
struct PDV_STRUCT {
	...
	std::vector<Byte> Image_Vec, Zip_Vec;
...
void Check_Image_File(PDV_STRUCT& pdv, std::ifstream& image_ifs, std::ifstream& zip_ifs) {
	...
	// Manually modifying the dimensions (1% increase or decrease) of the image will resolve the issue. Repeat if necessary.
	...
	while (ihdr_chunk_index++ != 32) { // We start checking from the 19th character position of the IHDR chunk within vector "Image_Vec".
		for (int i = 0; i < 7; i++) {
			if (pdv.Image_Vec[ihdr_chunk_index] == pdv.BAD_CHAR[i]) { // "BAD_CHAR" character found, display error message and exit program.
				std::cerr <<
...
void Check_Zip_File(PDV_STRUCT& pdv, std::ifstream& zip_ifs) {
	...
	int
		idat_chunk_length_field_index = 0,
		value_length_bits = 32;

	// Write the updated "IDAT" chunk length of vector "Zip_Vec" within its length field. 
	Value_Updater(pdv.Zip_Vec, idat_chunk_length_field_index, pdv.zip_size - 12, value_length_bits, pdv.big_endian);
...
void Complete_Extraction_Script(PDV_STRUCT& pdv) {
	...
	/* Extraction_Scripts_Array_Vec ""

	This vector stores an array of vectors containing shell/batch scripts used for extracting and opening the embedded zipped file (First filename or folder within the ZIP file record).

	Script supports both Linux & Windows. The completed script, when executed, will create the subdirectory ./pdvzip_extracted, move the image file into that directory, unzip the archive from the PNG image and then, 
	...
	constexpr int
		...
		WINDOWS_EXECUTABLE = 27,		// Just uses Windows default method to open the .exe file.
		BASH_SHELL = 28,			// After extraction, the script first uses chmod +x to give it executable permissions. then ./your_script_file.sh to run it. Windows uses set default program for this file type.
		Default = 29,				// Unknown extension. Uses OS default program to open unknown file types. Linux will use xdg-open command.
		...
		LINUX_EXECUTABLE = 31;			// After extraction, the script uses chmod +x to give the file executable permissions, then ./your_executable_file to run it.
	...
	// Python (.py), PowerShell (.ps1), Shell script (.sh) and executable (.exe). (no file extension, defaults to .exe, if not a folder).
	...
	pdv.Extraction_Scripts_Array_Vec = {

		// Script Vector 0 for common Video/Audio files.
		{	0x00, 0x00, 0x01, 0xFA, 0x69, 0x43, 0x43, 0x50, 0x44, 0x56, 0x5A, 0x49, 0x50, 0x5F, 0x5F, 0x00, 0x00, 0x0D, 0x52, 0x45, 0x4D, 0x3B, 0x49, 0x54,
		...
		// Script Vector 1 for PDF.
		{	0x00, 0x00, 0x01, 0xAD, 0x69, 0x43, 0x43, 0x50, 0x44, 0x56, 0x5A, 0x49, 0x50, 0x5F, 0x5F, 0x00, 0x00, 0x0D, 0x52, 0x45, 0x4D, 0x3B, 0x49, 0x54,
			0x45, 0x4D, 0x3D, 0x22, 0x22, 0x3B, 0x44, 0x49, 0x52, 0x3D, 0x22, 0x70, 0x64, 0x76, 0x7A, 0x69, 0x70, 0x5F, 0x65, 0x78, 0x74, 0x72, 0x61, 0x63,
			0x74, 0x65, 0x64, 0x22, 0x3B, 0x4E, 0x55, 0x4C, 0x3D, 0x22, 0x2F, 0x64, 0x65, 0x76, 0x2F, 0x6E, 0x75, 0x6C, 0x6C, 0x22, 0x3B, 0x63, 0x6C, 0x65,
			0x61, 0x72, 0x3B, 0x6D, 0x6B, 0x64, 0x69, 0x72, 0x20, 0x2D, 0x70, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22, 0x3B, 0x6D, 0x76, 0x20, 0x22, 0x24,
			0x30, 0x22, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22, 0x3B, 0x63, 0x64, 0x20, 0x22, 0x24, 0x44, 0x49, 0x52, 0x22, 0x3B, 0x75, 0x6E, 0x7A, 0x69,
		...
		// Script Vector 2 for Python.
		{	0x00, 0x00, 0x02, 0x0B, 0x69, 0x43, 0x43, 0x50, 0x44, 0x56, 0x5A, 0x49, 0x50, 0x5F, 0x5F, 0x00, 0x00, 0x0D, 0x52, 0x45, 0x4D, 0x3B, 0x49, 0x54,
			0x45, 0x4D, 0x3D, 0x22, 0x22, 0x3B, 0x44, 0x49, 0x52, 0x3D, 0x22, 0x70, 0x64, 0x76, 0x7A, 0x69, 0x70, 0x5F, 0x65, 0x78, 0x74, 0x72, 0x61, 0x63,
			...
			0x6C, 0x73, 0x26, 0x65, 0x78, 0x69, 0x74, 0x29, 0x26, 0x65, 0x63, 0x68, 0x6F, 0x2E, 0x26, 0x65, 0x78, 0x69, 0x74, 0x0D, 0x0A, 0x1E, 0x0E, 0xCF,
			0x5C },

		// Script Vector 3 for PowerShell.
		{	0x00, 0x00, 0x02, 0x72, 0x69, 0x43, 0x43, 0x50, 0x44, 0x56, 0x5A, 0x49, 0x50, 0x5F, 0x5F, 0x00, 0x00, 0x0D, 0x52, 0x45, 0x4D, 0x3B, 0x44, 0x49,
			...
			0x68, 0x6F, 0x2E, 0x26, 0x65, 0x78, 0x69, 0x74, 0x29, 0x20, 0x45, 0x4C, 0x53, 0x45, 0x20, 0x28, 0x70, 0x6F, 0x77, 0x65, 0x72, 0x73, 0x68, 0x65,
			0x6C, 0x6C, 0x20, 0x2D, 0x45, 0x78, 0x65, 0x63, 0x75, 0x74, 0x69, 0x6F, 0x6E, 0x50, 0x6F, 0x6C, 0x69, 0x63, 0x79, 0x20, 0x42, 0x79, 0x70, 0x61,
			0x73, 0x73, 0x20, 0x2D, 0x46, 0x69, 0x6C, 0x65, 0x20, 0x22, 0x21, 0x49, 0x54, 0x45, 0x4D, 0x21, 0x22, 0x20, 0x21, 0x41, 0x52, 0x47, 0x53, 0x21,
			0x26, 0x65, 0x63, 0x68, 0x6F, 0x2E, 0x26, 0x65, 0x78, 0x69, 0x74, 0x29, 0x0D, 0x0A, 0x8A, 0x9B, 0x46, 0xE8 },

		// Script Vector 4 for Windows Executable.
		{	0x00, 0x00, 0x01, 0x4D, 0x69, 0x43, 0x43, 0x50, 0x44, 0x56, 0x5A, 0x49, 0x50, 0x5F, 0x5F, 0x00, 0x00, 0x0D, 0x52, 0x45, 0x4D, 0x3B, 0x44, 0x49,
			...
			0x7E, 0x78, 0x30, 0x22, 0x20, 0x2A, 0x2E, 0x70, 0x6E, 0x67, 0x26, 0x22, 0x22, 0x20, 0x26, 0x65, 0x63, 0x68, 0x6F, 0x2E, 0x26, 0x65, 0x78, 0x69,
			0x74, 0x0D, 0x0A, 0xCC, 0xC6, 0x99, 0x5E },

		// Script Vector 5 for Bash Shell.
		...
		// Script Vector 8 for unmatched file extensions/unknown file type. Rely on the OS efault application to deal with these files.
		{	 }

	};

	// pdv.Case_Values_Vec, will contain between 3 and 5 values depending on matched file extension.
	// The first value in the vector is always the script vector within pdv.Extraction_Scripts_Array_Vec.
	// The remaining values are insert index locations within the script vector for elements to be inserted, such as filenames & command arguments.

	switch (extension_index) {
		case VIDEO_AUDIO: {
			pdv.Case_Values_Vec = { 0, 484, 28 };
			break;
		}
		case PDF: {
			pdv.Case_Values_Vec = { 1, 406, 28 };
			break;
		}
		case PYTHON: {
			pdv.Case_Values_Vec = { 2, 267, 257, 188, 28 };
			break;
		}
		case POWERSHELL: {
			pdv.Case_Values_Vec = { 3, 261, 251, 182, 51 };
			break;
		}
		case WINDOWS_EXECUTABLE: {
			pdv.Case_Values_Vec = { 4, 278, 276 };
			break;
		}
		case BASH_SHELL: {
			pdv.Case_Values_Vec = { 5, 308, 306, 142, 28 };
			break;
		}

		case FOLDER: {
			pdv.Case_Values_Vec = { 6, 329, 28 };
			break;
		}

		case LINUX_EXECUTABLE: {
			pdv.Case_Values_Vec = { 7, 142, 28 };
			break;
		}

		default: {
			pdv.Case_Values_Vec = { 8, 295, 28 };
		}
	}

	const int SCRIPT_VECTOR = pdv.Case_Values_Vec[0];

	// Update the relevant script vector with the required filenames and command arguments.

	if (extension_index == WINDOWS_EXECUTABLE) {
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[1], args_windows.begin(), args_windows.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[2], first_zip_name.begin(), first_zip_name.end());
	}
	else if (extension_index == LINUX_EXECUTABLE) {
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[1], args_linux.begin(), args_linux.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[2], first_zip_name.begin(), first_zip_name.end());
	}
	else if (extension_index == PYTHON || extension_index == POWERSHELL || extension_index == BASH_SHELL) {
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[1], args_windows.begin(), args_windows.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[2], first_zip_name.begin(), first_zip_name.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[3], args_linux.begin(), args_linux.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[4], first_zip_name.begin(), first_zip_name.end());
	} 
	else {
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[1], first_zip_name.begin(), first_zip_name.end());
		pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.Case_Values_Vec[2], first_zip_name.begin(), first_zip_name.end());
	}

	pdv.script_size = pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].size();

	int
		iccp_chunk_length_field_index = 2,
	...
	Value_Updater(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR], iccp_chunk_length_field_index, pdv.script_size - 12, value_length_bits, pdv.big_endian);
	...
	for (int i = 0; i < 7; i++) {
		if (pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR][3] == pdv.BAD_CHAR[i]) {

			// "BAD_CHAR" found. Insert 10 bytes string at the end of "Script_Vector" to increase chunk length. Update chunk length field. 
			// This should now skip over any BAD_CHAR characters, regardless of the new chunk size.

			const std::string INCREASE_LENGTH_STRING = "..........";

			pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].insert(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].begin() + pdv.script_size - 4, INCREASE_LENGTH_STRING.begin(), INCREASE_LENGTH_STRING.end());
			pdv.script_size = pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].size();

			Value_Updater(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR], iccp_chunk_length_field_index, pdv.script_size - 12, value_length_bits, pdv.big_endian); // Update size again.

			break;
		}
	}

	pdv.combined_file_size = pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR].size() + pdv.Image_Vec.size() + pdv.Zip_Vec.size();

	constexpr int
		MAX_SCRIPT_SIZE = 1500,
		ICCP_CHUNK_START_INDEX_FIELD = 4;  // When calculating the CRC value for a PNG chunk, we always begin at and include the chunk name field.
	...
	// Pass these two values (ICCP_CHUNK_START_INDEX_FIELD & iCCP chunk size (script_size - 8) to the CRC function to get correct "iCCP" chunk CRC value.

	const size_t ICCP_CHUNK_CRC_VALUE = Crc(&pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR][ICCP_CHUNK_START_INDEX_FIELD], pdv.script_size - 8);

	// Get vector start index location for the "iCCP" chunk's 4 byte CRC value field.
	size_t iccp_crc_value_index_field = pdv.script_size - 4;

	value_length_bits = 32;

	// Write the updated CRC value into the "iCCP" chunk's CRC field (4 bytes) within vector "Script_Vec".
	Value_Updater(pdv.Extraction_Scripts_Array_Vec[SCRIPT_VECTOR], iccp_crc_value_index_field, ICCP_CHUNK_CRC_VALUE, value_length_bits, pdv.big_endian);
...
void Combine_Vectors(PDV_STRUCT& pdv) {
	...
	// This value will be used as the insert location within vector "Image_Vec" for contents of "Script_Vector". 
	// Script_Vector's inserted contents will appear within the "iCCP" chunk, just after the "IHDR" chunk of "Image_Vec".

	constexpr int FIRST_IDAT_INSERT_INDEX_FIELD = 33;

	std::cout << "\nEmbedding extraction script within the PNG image.\n";

	// Insert contents of "Script_Vector" (the "iCCP" chunk containing the extraction script) into vector "Image_Vec".	
	pdv.Image_Vec.insert((pdv.Image_Vec.begin() + FIRST_IDAT_INSERT_INDEX_FIELD), pdv.Extraction_Scripts_Array_Vec[pdv.Case_Values_Vec[0]].begin(), pdv.Extraction_Scripts_Array_Vec[pdv.Case_Values_Vec[0]].end());
	...
	const size_t LAST_IDAT_CHUNK_INDEX_FIELD = pdv.image_size + pdv.script_size - 8;
	...
	Fix_Zip_Offset(pdv, LAST_IDAT_CHUNK_INDEX_FIELD);

	// Get CRC value for our (new) last "IDAT" chunk.
	const size_t LAST_IDAT_CHUNK_CRC_VALUE = Crc(&pdv.Image_Vec[LAST_IDAT_CHUNK_INDEX_FIELD], pdv.zip_size - 8); // We don't include the length or CRC fields (-8 bytes).

	pdv.image_size = pdv.Image_Vec.size();

	size_t last_idat_crc_insert_field_index = pdv.image_size - 16;

	int value_length_bits = 32;

	pdv.big_endian = true;

	// Write new CRC value into the last "IDAT" chunk's CRC index field, within the vector "Image_Vec".
	Value_Updater(pdv.Image_Vec, last_idat_crc_insert_field_index, LAST_IDAT_CHUNK_CRC_VALUE, value_length_bits, pdv.big_endian);

	Write_Out_Polyglot_File(pdv);
}

void Fix_Zip_Offset(PDV_STRUCT& pdv, const size_t& LAST_IDAT_CHUNK_INDEX_FIELD) {
	...
	const size_t
		START_CENTRAL_DIR_INDEX = std::search(pdv.Image_Vec.begin() + LAST_IDAT_CHUNK_INDEX_FIELD, pdv.Image_Vec.end(), START_CENTRAL_DIR_SIG.begin(), START_CENTRAL_DIR_SIG.end()) - pdv.Image_Vec.begin(),
	...
	while (zip_records--) {
		...
		Value_Updater(pdv.Image_Vec, central_local_index, new_offset, value_length_bits, pdv.big_endian);
	}
	...
	Value_Updater(pdv.Image_Vec, end_central_start_index, START_CENTRAL_DIR_INDEX, value_length_bits, pdv.big_endian);
...
For common video/audio files, Linux uses the media players vlc or mpv. Windows uses the set default media player.
PDF '.pdf', Linux uses either evince or firefox. Windows uses the set default PDF viewer.
Python '.py', Linux & Windows use python3 to run these programs.
PowerShell '.ps1', Linux uses pwsh command (if PowerShell installed). 
