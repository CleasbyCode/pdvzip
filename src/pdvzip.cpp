...
void startPdv(const std::string& IMAGE_NAME, const std::string& ZIP_NAME, bool isZipFile) {
	...
	constexpr uint_fast8_t 
		...
		MIN_INZIP_NAME_LENGTH = 4,
		ZIP_RECORD_FIRST_FILE_NAME_LENGTH_INDEX = 0x22, 
		ZIP_RECORD_FIRST_FILE_NAME_INDEX = 0x26,
		VIDEO_AUDIO = 23,
		PDF = 24, 
		PYTHON = 25, 
		POWERSHELL = 26, 
	...
	if (!std::equal(std::begin(ZIP_SIG), std::end(ZIP_SIG), std::begin(Idat_Zip_Vec) + 8) || 
			MIN_INZIP_NAME_LENGTH > INZIP_NAME_LENGTH) {
				std::cerr << "\nZIP File Error: " << (MIN_INZIP_NAME_LENGTH > INZIP_NAME_LENGTH ? 
	...
	const uint_fast16_t CHECK_FOR_FILE_EXTENSION = static_cast<uint_fast16_t>(ZIP_RECORD_FIRST_FILE_NAME.find_last_of('.'));
