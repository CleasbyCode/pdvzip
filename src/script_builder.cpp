...
const ScriptTemplate& getScriptTemplate(FileType file_type) {
	static const std::unordered_map<FileType, ScriptTemplate> TEMPLATES = {
		...
		},
		{
			FileType::PDF,
			{
				R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_extracted";NUL="/dev/null";clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";hash -r;if command -v evince >$NUL 2>&1;then clear;evince "$ITEM" &> $NUL;else firefox "$ITEM" &> $NUL;clear;fi;exit;)"sv,
				R"(#&cls&setlocal EnableDelayedExpansion&set "DIR=pdvzip_extracted"&mkdir ".\!DIR!"&move "%~dpnx0" ".\!DIR!"&cd ".\!DIR!"&cls&tar -xf "%~n0%~x0"&ren "%~n0%~x0" *.png&start "" {{WINDOWS_FILENAME_ARG}}&exit)"sv
			}
		},
		{
			FileType::PYTHON,
			{
				R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_extracted";clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";hash -r;if command -v python3 >/dev/null 2>&1;then clear;python3 "$ITEM" {{LINUX_ARGS}};else clear;fi;exit;)"sv,
		...
		},
		{
			FileType::POWERSHELL,
			{
				R"(DIR="pdvzip_extracted";ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";hash -r;if command -v pwsh >/dev/null 2>&1;then clear;pwsh "$ITEM" {{LINUX_ARGS}};else clear;fi;exit;)"sv,
		...
		},
		{
			FileType::BASH_SHELL,
			{
				R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_extracted";clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";chmod +x -- "$ITEM";"$ITEM" {{LINUX_ARGS}};exit;)"sv,
		...
		},
		{
			...
			{
				R"(SELF=$(basename -- "$0");DIR="pdvzip_extracted";clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";clear;exit;)"sv,
		...
		},
		{
			...
			{
				R"(ITEM={{LINUX_FILENAME_ARG}};SELF=$(basename -- "$0");DIR="pdvzip_extracted";clear;mkdir -p "$DIR";mv -- "$0" "$DIR";cd "$DIR";unzip -qo -- "$SELF";chmod +x -- "$ITEM";"$ITEM" {{LINUX_ARGS_COMBINED}};exit;)"sv,
