...
[[nodiscard]] vBytes makeIccpChunkSkeleton() {
	return vBytes{
...
void insertScriptText(vBytes& script_vec, std::string_view script_text) {
	script_vec.insert(
...
[[nodiscard]] std::size_t writeChunkLength(vBytes& script_vec) {
	const std::size_t chunk_data_size = script_vec.size() - CHUNK_FIELDS_COMBINED_LENGTH;
...
void padChunkLengthUntilLinuxSafe(vBytes& script_vec, std::size_t& chunk_data_size) {
	...
	while (isLinuxProblemMetacharacter(script_vec[LENGTH_FIRST_BYTE_INDEX])) {
		if (++pad_attempts > MAX_PAD_ATTEMPTS) {
			throw std::runtime_error("Script Error: Could not make iCCP chunk length Linux-safe.");
		}

		const auto pad_position = script_vec.begin()
			+ static_cast<vBytes::difference_type>(chunk_data_size + PAD_OFFSET);
		script_vec.insert(
			pad_position,
			PAD.begin(),
			PAD.end());
...
vBytes buildExtractionScript(
	...
	vBytes script_vec = makeIccpChunkSkeleton();
	...
	insertScriptText(script_vec, script_text);
	std::size_t chunk_data_size = writeChunkLength(script_vec);
