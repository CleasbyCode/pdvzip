...
[[nodiscard]] PngIhdr readPngIhdr(std::span<const Byte> png_data) {
	...
	if (getValue(png_data, IHDR_LENGTH_INDEX, 4) != 13) {
...
void validateInputPngForDecode(const PngIhdr& ihdr) {
	constexpr std::size_t
		MIN_DIMENSION      = 68,
		MAX_DIMENSION      = 4096,
		MAX_DECODE_PIXELS  = static_cast<std::size_t>(MAX_DIMENSION) * MAX_DIMENSION,
		MAX_DECODE_BYTES   = MAX_DECODE_PIXELS * 4;
	...
	const std::size_t pixel_count = checkedMultiply(
		ihdr.width,
		ihdr.height,
		"Image Error: PNG pixel count overflow.");
	if (pixel_count > MAX_DECODE_PIXELS) {
		throw std::runtime_error("Image File Error: Cover image dimensions exceed the supported limit.");
	}

	const std::size_t decode_bytes = checkedMultiply(
		pixel_count,
		4,
		"Image Error: PNG decode buffer size overflow.");
	if (decode_bytes > MAX_DECODE_BYTES) {
		throw std::runtime_error("Image Error: Decoded image exceeds the safe memory budget.");
	}
}
