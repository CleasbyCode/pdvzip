...
int main(int argc, char** argv) {
	try {
    	...
    	auto copy_chunk_type = [&](const auto& chunk_signature) {
        	...
        	while (true) {
            	...
            	for (uint8_t i = 0; i < CHUNK_LENGTH_FIELD_SIZE; ++i) {
        			chunk_length = (chunk_length << 8) | static_cast<uint32_t>(image_vec[chunk_length_pos + i]);
    			}
		...
		if (std::find(LINUX_PROBLEM_CHARACTERS.begin(), LINUX_PROBLEM_CHARACTERS.end(), ICCP_CHUNK_LENGTH_FIRST_BYTE_VALUE) != LINUX_PROBLEM_CHARACTERS.end()) {
