...
int main(int argc, char** argv) {
	try {
		...
		const std::string ARC_RECORD_FIRST_FILENAME{archive_vec.begin() 
			+ ARC_RECORD_FIRST_FILENAME_INDEX, archive_vec.begin() + ARC_RECORD_FIRST_FILENAME_INDEX + ARC_RECORD_FIRST_FILENAME_LENGTH};
		...
		const uint32_t END_CENTRAL_DIR_INDEX = COMPLETE_IMAGE_SIZE - static_cast<uint32_t>(std::distance(image_vec.rbegin(), std::search(image_vec.rbegin(), image_vec.rend(), 
				END_CENTRAL_DIR_SIG.rbegin(), END_CENTRAL_DIR_SIG.rend()))) - ZIP_SIG_LENGTH;
		uint32_t
		...
		while (total_zip_records--) {
			...
			central_dir_local_index = searchSig(image_vec, central_dir_local_index, INCREMENT_NEXT_SEARCH_POS, START_CENTRAL_DIR_SIG) + CENTRAL_LOCAL_INDEX_DIFF; // Next central_dir index.
		}
